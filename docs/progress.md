...
### Implementat
- `Users` are acum `accountStatus` (`active`, `pending_approval`, `rejected`, `disabled`) + `invitedBy`, `approvedBy`, `approvedAt`.
- Admin Firmă poate crea/propunere utilizatori în Payload admin, dar hook-ul îi forțează pe firma proprie și în `pending_approval`; nu poate crea `super_admin`.
- Super Admin poate activa/respinge utilizatorii prin `accountStatus`; activarea setează automat `approvedBy` și `approvedAt`.
- Simulările salvate, lista `/simulari` și PDF export refuză utilizatorii non-active (`pending_approval`, `rejected`, `disabled`) cu mesaj clar.
- Admin dashboard are card „Aprobări utilizatori”: Super Admin vede toate conturile pending, Admin Firmă vede doar firma proprie.
- Adăugat migration SQL repo-tracked: `web/scripts/migrations/2026-05-16-commercial-beta-users.sql`.

### Verificare
- Neon production `instrumentar` (`falling-queen-72767820`, `neondb`) — migration SQL aplicat; `users.account_status`, approval refs, indexes și FK-uri verificate; userii existenți au rămas `active`.
- `npx tsc --noEmit` în `web` — verde.
- `npx tsx scripts/check-simulari-access.ts` — verde, include pending/disabled access checks.
